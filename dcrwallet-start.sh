...
function startDaemon(){
	nohup $BIN_PATH/$BIN $NET --debuglevel=$DEBUG_LEVEL --logdir=$LOGDIR > $OUTFILE 2>&1 &
	sleep 4
}

function startConsole(){
        nohup $BIN_PATH/$BIN $NET --debuglevel=$DEBUG_LEVEL --logdir=$LOGDIR
}
